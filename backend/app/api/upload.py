...
from app.core.config import settings  # noqa: F401 (may be used via env-driven behavior)
...
async def delete_document_endpoint(
    ...
    try:
        ...
        if user_role != UserRole.ADMIN.value and document_owner != current_user["id"]:
            logger.warning(
                f"Unauthorized delete attempt: user {current_user['id']} tried to delete document {document_id} owned by {document_owner}"
            )
        ...
        delete_success = supabase_retriever.delete_documents_by_filename(filename)

        # Verify deletion worked
        post_stats = supabase_retriever.get_collection_stats()
...
async def reset_knowledge_base(
    ...
    try:
        ...
        pre_stats = supabase_retriever.get_collection_stats()
        ...
        reset_success = supabase_retriever.reset_collection()
